    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text("Brief Stats",style: GoogleFonts.workSans(fontSize: 20,fontWeight: FontWeight.bold)),
                  SizedBox(height: 20,),
                  PieChart(
                    dataMap: dataMap,
                    ...
                    colorList: [Colors.blue.shade100,Colors.blue.shade400,Colors.blue.shade700,Colors.blue.shade900],
                    ...
                    chartValuesOptions: ChartValuesOptions(
                      ...
                      chartValueStyle: GoogleFonts.workSans(fontSize: 20,fontWeight: FontWeight.normal,color: Colors.black),
                ...
                child: Column(
                  ...
                  children: [
                   ...
                   SizedBox(height: 10,),
                    ...
                    SizedBox(height: 10,),
                    status.indexOf(data[pos]['status'])==1||status.indexOf(data[pos]['status'])==2?Row(
                      children: [
                        status.indexOf(data[pos]['status'])==1?Text("ETA for Partner:- ${DateFormat.yMMMMd().add_jm().format(
                            DateTime.fromMillisecondsSinceEpoch(
                                data[pos]['eta']))}",style: GoogleFonts.workSans(fontSize: 15,fontWeight: FontWeight.normal),maxLines: 1,overflow: TextOverflow.ellipsis,):Text("ETA for Rider:- ${DateFormat.yMMMMd().add_jm().format(
                            DateTime.fromMillisecondsSinceEpoch(
                                data[pos]['eta']))}",style: GoogleFonts.workSans(fontSize: 15,fontWeight: FontWeight.normal),maxLines: 1,overflow: TextOverflow.ellipsis,),
                      ],
                    ):Container(),
                    status.indexOf(data[pos]['status'])==1||status.indexOf(data[pos]['status'])==2?SizedBox(height: 10,):Container(),
                    Row(
                      children: [
                      ...
                      child: StepProgressIndicator(
                        ...
                        customStep: (index, color, _) => Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Icon(
                                status.indexOf(data[pos]['status'])==index?(Icons.person):status.indexOf(data[pos]['status'])>index?Icons.check:Icons.remove,
                                color: status.indexOf(data[pos]['status'])==index?(Colors.green):status.indexOf(data[pos]['status'])>index?Colors.white:Colors.black,
                              ),
                              SizedBox(height: 5,),
                              Text(status[index],style: status.indexOf(data[pos]['status'])==index?GoogleFonts.workSans(color: Colors.green,fontWeight: FontWeight.bold):status.indexOf(data[pos]['status'])>=index?GoogleFonts.workSans(color: Colors.white):GoogleFonts.workSans(color: Colors.black),)
                            ],
                          ),
                        )
                      ),
          ...
          },itemCount: data.length,physics: NeverScrollableScrollPhysics(),shrinkWrap: true,),
        ],
      ),
    );
  }
